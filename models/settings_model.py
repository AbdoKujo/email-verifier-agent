...
class SettingsModel:
    ...
    def configure_browser_settings(self) -> None:
        ...
        if human_behavior.lower() == 'y':
            ...
            print("\nHuman behavior enabled - verification will use realistic typing and delays")
        else:
            self.set("human_behavior_enabled", "False", False)
            print("\nHuman behavior disabled - verification will be faster but may be less stealthy")
    ...
    def configure_rate_limiting_settings(self) -> None:
        ...
        while True:
            ...
            print("1. Configure SMTP rate limiting")
            print("2. Configure Selenium rate limiting")
            print("3. Configure API rate limiting")
            print("4. Configure bounce verification settings")
            ...
            if choice == "1":
                # SMTP rate limiting
                smtp_delay = input("\nEnter delay between SMTP checks in seconds (current: {}): ".format(
                    self.get("smtp_delay", "2")
                ))
                if smtp_delay:
                    try:
                        smtp_delay = max(1, int(smtp_delay))
                        self.set("smtp_delay", str(smtp_delay), True)
                        print(f"\nSMTP delay set to {smtp_delay} seconds")
                    except ValueError:
                        print("\nInvalid input. Using default value.")
            
            elif choice == "2":
                # Selenium rate limiting
                selenium_delay = input("\nEnter delay between Selenium checks in seconds (current: {}): ".format(
                    self.get("selenium_delay", "5")
                ))
                if selenium_delay:
                    try:
                        selenium_delay = max(1, int(selenium_delay))
                        self.set("selenium_delay", str(selenium_delay), True)
                        print(f"\nSelenium delay set to {selenium_delay} seconds")
                    except ValueError:
                        print("\nInvalid input. Using default value.")
            
            elif choice == "3":
                # API rate limiting
                api_delay = input("\nEnter delay between API checks in seconds (current: {}): ".format(
                    self.get("api_delay", "1")
                ))
                if api_delay:
                    try:
                        api_delay = max(1, int(api_delay))
                        self.set("api_delay", str(api_delay), True)
                        print(f"\nAPI delay set to {api_delay} seconds")
                    except ValueError:
                        print("\nInvalid input. Using default value.")
            
            elif choice == "4":
                # Bounce verification settings
                bounce_wait_time = input("\nEnter wait time for bounce-backs in seconds (current: {}): ".format(
                    self.get("bounce_wait_time", "60")
                ))
                if bounce_wait_time:
                    try:
                        bounce_wait_time = max(10, int(bounce_wait_time))
                        self.set("bounce_wait_time", str(bounce_wait_time), True)
                        print(f"\nBounce wait time set to {bounce_wait_time} seconds")
                    except ValueError:
                        print("\nInvalid input. Using default value.")
